...
class SapReceiver(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            ip, port = address
            if DEBUG : print('received bytes from:'+ str(len(data)) +", "+ ip +"/"+ str(port) + "\n")
            type = data[0] & 0x04
    ...
    def get_video_props(self, a_line_from_sdp):
        ...
        if elements[0] == 'a=fmtp:'+str(self.payload_type):
            if self.encoding_type == "H264":
                ...
                new_params = ""
                new_params += ', '.join(x.replace('=','=(string)') for x in params)
                result += new_params
